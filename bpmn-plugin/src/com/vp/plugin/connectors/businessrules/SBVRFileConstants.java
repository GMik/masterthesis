package com.vp.plugin.connectors.businessrules;

public class SBVRFileConstants {

	public static final String TYPE_AND_DATA_SEPARATOR = ":";
	public static final String DATA_SEPARATOR = ";";
	public static final int TYPE_POSITION = 0;
	public static final int DATA_POSITION = 1;

	public static final String TYPE_TERM = "Term";
	public static final String TYPE_FACT = "Fact";
	public static final String TYPE_CLASS_ATTRIBUTE_TERM = "ClassAttributeTerm";
	public static final String TYPE_CLASS_ATTRIBUTE_FACT = "ClassAttributeFact";
	public static final String TYPE_BUSINESS_RULE = "BusinessRule";

	// public static final String CLASS_HAS_ATTRIBUTE_RELATION = "has";
	// public static final String ATTRIBUTE_IS_PROPERTY_OF_CLASS_RELATION = "is
	// property of";
	// public static final String CLASS_X_GENERALIZES_CLASS_Y = "generalizes";

}
