...
public class ReloadClassesShapeController implements VPShapeController {
	...
	private Shape createShape(VPShapeInfo aShapeInfo) {
		...
		if (true) { // <---- change this to FALSE to draw another shape
			...
			lPath.append(new Arc2D.Double(lXUnit * 6.5, lYUnit * 12, lXUnit * 3, lYUnit * 3, 180, 180, Arc2D.OPEN),
					false);

			return lPath;
		}

		// return new Rectangle((int) lBounds.getX(), (int) lBounds.getY(),
		// (int) lBounds.getWidth(), (int) lBounds.getHeight());
	}
